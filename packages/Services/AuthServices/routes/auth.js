        ...
        userModel.find({ UserName: username }, (err, data) => {
            if (err) {
                if (lib.isEmptyObject(err)) {
                    reject(lib.error(401, "user name not found in database"))
                } else {
                    reject(lib.error(500, "internal server error", err))
                }
            } else {
                console.log(JSON.stringify(data[0]))
                if (lib.isEmptyObject(data[0])) {
                    reject(lib.error(401, "user name not found in database"))
                } else {
                    if (checkSecretKey(password, data[0].Password)) {
                        resolve(data)
                    } else {
                        reject(lib.error(401, "password does not match"))
                    }
                }
            }
        });
