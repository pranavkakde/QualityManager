...
const TestCaseList = () => {
  ...
  const filteredCases = cases.filter(c => {
    ...
    if (filterTag && !c.tag?.toLowerCase().includes(filterTag.toLowerCase())) {
      return false;
    }
    if (filterAuthor) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        ...
        <p className="text-slate-500 font-medium">Loading Suite Test Cases...</p>
      </div>
    );
  }
    ...
    <div className="p-8">
      ...
      {suite && (
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 mb-8">
          <div className="flex justify-between items-start gap-4">
            <div>
              <span className="text-[10px] font-black tracking-widest text-indigo-600 uppercase bg-indigo-50 px-2 py-0.5 rounded border border-indigo-200 mb-2 inline-block">
                Suite Workspace
              </span>
              <h1 className="text-2xl font-extrabold text-slate-800 mb-2">{suite.name}</h1>
              <p className="text-slate-500 max-w-2xl leading-relaxed">{suite.description}</p>
            </div>
            <button
              onClick={() => navigate(`/suites/${testsuiteid}/cases/add`)}
              className="bg-indigo-600 text-white px-5 py-2.5 rounded-xl font-bold hover:bg-indigo-700 transition-all shadow-sm flex items-center gap-2 text-sm"
            >
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 mb-8">
        <h2 className="text-sm font-bold text-slate-800 uppercase tracking-wider mb-4 flex items-center gap-2">
          <Search size={16} className="text-slate-400" /> Filters
        </h2>
        ...
        {/* Quick info row */}
        <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100 text-xs font-bold text-slate-500">
          <div>
            Suite Total: <span className="text-slate-800">{cases.length}</span>
            <span className="mx-2">|</span>
            Filtered: <span className="text-indigo-600">{filteredCases.length}</span>
          </div>
          {(filterName || filterDesc || filterVersion || filterStatus !== 'all' || filterAuthor || filterTag) && (
            <button
              onClick={() => {
                setFilterName('');
                setFilterDesc('');
              ...
              className="text-rose-600 hover:text-rose-700 font-extrabold flex items-center gap-1"
            >
      ...
      <h2 className="text-lg font-bold text-slate-800 mb-4">Test Cases ({filteredCases.length})</h2>
      ...
      {cases.length === 0 ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center shadow-sm">
          ...
          <h3 className="font-bold text-slate-800 text-lg mb-1">No Test Cases Match Filters</h3>
          <p className="text-slate-500 max-w-sm mx-auto mb-6">Try clearing your filters or create a new testcase inside this suite.</p>
          <button
            onClick={() => {
              setFilterName('');
              setFilterDesc('');
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredCases.map((testcase) => (
                  <tr key={testcase.testcaseid} className="hover:bg-slate-50/50 transition-colors text-slate-700 text-sm">
                    ...
                    <td className="py-4 px-6">
                      {testcase.tag ? (
                        <span className="inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded bg-slate-100 text-slate-600 border border-slate-200">
                          🏷️ {testcase.tag}
                        </span>
                      ) : (
